...
let samples =
    ...
    <| seq {
        ...
        yield "POST / HTTP/1.1", [ { Name = "Host"; Values = [ "example.org" ] } ], exampleOrg
        yield "OPTIONS * HTTP/1.1", [ { Name = "Host"; Values = [ "example.org" ] } ], exampleOrg
        yield "GET / HTTP/1.1", [ { Name = "Host"; Values = [ "localhost:8080" ] } ], localhost8080
    }
